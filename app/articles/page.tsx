...
import { ArticlesProps } from '@/components/partials/landing/Articles'
...
const Page = async({ categorySlug }: ArticlesProps) => {
    ...
    return (
        <section id="articles" className="px-8 lg:px-12 xl:px-32 2xl:px-44 py-20 min-h-[60vh] h-inherit pt-35">
            <div className="w-full xl:w-1/2" data-aos="fade-up" data-aos-delay="50" >
                <h1 className="text-5xl font-outfit font-medium leading-[1em] text-eee-dark-gray">Articles</h1>
            ...
            {posts.map((post: any) => {
                ...
                return (
                <article key={post.id} className="" data-aos="fade-up" data-aos-delay="50">
                    {/* <img src={image} alt="" className="rounded w-full max-w-full" />} */}
                    {image && 
                    <Link href={`/articles/${post.slug}`} >
                    <div className="relative w-full h-60 xl:h-110 rounded overflow-hidden bg-eee-light-gray/50">
                        <Image
                        src={image}
